...
Your June 2026 help article says that a key created under **Code › Vibe Code CLI** can be a Vibe plan key tied to the Vibe budget and can return 401, while API automation should use a Studio pay-as-you-go key. Could you confirm from internal metadata: this key's product type (`Studio` versus `Vibe`), issue and expiry timestamps, Workspace, explicit revocation/expiry reason, and whether Vibe plan keys have a one-hour TTL? I need a non-expiring Studio evaluation key rather than a short-lived Vibe/plan credential. My account ID is `76a1b995-3912-40f0-8ab5-a25ce3341e1f` and organization ID is `f1cd0a1c-0189-4136-b6b2-af4bec85a0bc`.
