...
Separately, I instrumented the API-key failure at 30-second resolution. The replacement credential was stored in my secret manager at `11:10:21.332866Z`. The same in-memory credential returned 200 on `/v1/models` at `12:09:48Z`, then 401 at `12:10:14Z`: a 25.765-second transition window and only 59 minutes 52.84 seconds after the secret-manager revision. A second process using the same fixed credential from the benchmark namespace changed from 200 to 401 one second later. A fresh read from the secret manager then returned 401 for both `/v1/models` and chat. Both network paths had the same egress, so this was not a local secret rollback, client-header issue, or IP-specific edge response.

Immediately before expiry, the API advertised `labs-leanstral-1-5` limits of 30,000,000 tokens/minute and 300 requests/minute, with almost the entire window remaining. A controlled concurrent request burst returned only 200 responses. The failure returned no rate-limit headers or `Retry-After`, only `401 {"detail":"Unauthorized"}`. This strongly resembles an approximately one-hour credential TTL rather than API rate limiting, token exhaustion, or an account ban.

The Admin key list confirms that the affected replacement is type **Studio**, expiration **Never**, with the default **Shared connectors only** scope. Your documentation says this scope controls Connector access only; it should not restrict ordinary model/chat API calls. Therefore the documented Vibe-plan-key explanation does not apply here. Please investigate whether the auth data plane has an internal `expires_at`, hourly cache/entitlement revalidation, or Workspace/Organization state inconsistent with the Admin control plane. In particular, please confirm the key's internal issue/expiry timestamps and explicit revocation reason, and correlate the supplied transition CF-Ray IDs. My account ID is `76a1b995-3912-40f0-8ab5-a25ce3341e1f` and organization ID is `f1cd0a1c-0189-4136-b6b2-af4bec85a0bc`.
